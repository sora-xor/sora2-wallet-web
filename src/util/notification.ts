...
const getVapidKeys = () => {
  ...
  return {
    publicKey: 'BN_yHElgeBlW7lf-G-ihLKfANydD94itINIG7eoqQdMNev7r-30su1Buek1bnFORpywB_EppkujbCtrYUFnMTio',
    privateKey: 'aBGBrP5P31P3Cmg_G4xYR5tDpY-IEc4FoFYhYFWbLIA',
  };
...
const uint8ArrayToBase64 = (arr: Uint8Array): string => Buffer.from(arr).toString('base64');

async function registerServiceWorker(publicKey: string): Promise<any> {
  const registration = await navigator.serviceWorker.ready;
  const isSubscription = await registration.pushManager.getSubscription();
  let pushSubscription;

  try {
    if (!isSubscription) {
      pushSubscription = await setSubscription(publicKey);
    } else {
...
async function setSubscription(publicKey: string): Promise<any> {
...
export async function pushNotification(asset: Asset, isAssetDeposited: boolean): Promise<void> {
  ...
  if (Notification.permission === 'granted') {
    if ('serviceWorker' in navigator) {
      ...
      try {
        ...
        const subscription = await registerServiceWorker(publicKey);
        ...
        //   window.location.href = 'https://polkaswap.io/';
        // };
