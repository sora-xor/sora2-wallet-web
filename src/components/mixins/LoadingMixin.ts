...
export default class LoadingMixin extends Vue {
  ...
  async withLoading(func: AsyncVoidFn | VoidFunction): Promise<void> {
    this.loading = true;
    try {
      return await func();
    } catch (e) {
   ...
   * @param func
   */
  async withApi(func: AsyncVoidFn | VoidFunction): Promise<void> {
    this.loading = true;
  ...
  async withParentLoading(func: AsyncVoidFn): Promise<void> {
