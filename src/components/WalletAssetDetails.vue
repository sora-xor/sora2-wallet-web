<template>
  <wallet-base :title="currentRouteParams.symbol" show-back show-settings @back="handleBack">
    <div v-if="selectedAssetDetails.length > 0" class="asset-details s-flex">
      <div
        class="asset-details-item s-flex"
        v-for="(item, index) in selectedAssetDetails"
        :key="`asset-details-${index}`"
      >
        <div class="info s-flex">
          <div class="info-operation">{{ item.operation }}</div>
          <div class="info-text">
            <span>{{ `${item.fromAmount} ${currentRouteParams.symbol} for ${item.toAmount} ${item.toSymbol}` }}</span>
            <s-button
    ...
    <div v-else class="asset-details-empty">{{ t('assets.emptyDetails') }}</div>
  </wallet-base>
</template>
