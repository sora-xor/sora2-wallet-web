...
const createStore = (env: SoraNetwork) => new Vuex.Store({
  modules: {
    Settings: {
      getters: {
        soraNetwork: () => env
...
useDescribe('TransactionHashView.vue', TransactionHashView, () => {
  ...
  MOCK_TRANSACTION_HASH_VIEW.map(item => it(`[type: ${item.type}, env: ${SoraNetwork.Dev}]: should be rendered correctly`, () => {
    const wrapper = shallowMount(TransactionHashView, {
      localVue,
      propsData: item,
      store: createStore(SoraNetwork.Dev)
    })
    const txLinks = wrapper.findAll('.s-input-container .transaction-link').wrappers.map(item => item.element) as Array<HTMLAnchorElement>
  ...
  MOCK_TRANSACTION_HASH_VIEW.map(item => it(`[type: ${item.type}, env: ${SoraNetwork.Prod}]: should be rendered correctly`, () => {
    const wrapper = shallowMount(TransactionHashView, {
      localVue,
      propsData: item,
